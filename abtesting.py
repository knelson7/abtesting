...
def get_stdev(nums):
    ...
    quotient = sum_deviations/(len(nums)-1)
    std_dev = math.sqrt(quotient)
    return std_dev
...
def get_2_sample_df(a, b):
    ...
    first_term = ((get_stdev(a) ** 2 / len(a)) **2) / (len(a)-1)
    second_term = ((get_stdev(b) ** 2 / len(b)) **2) / (len(b)-1)
...
def get_expected_grid(observed_grid):
    ...
    ** DO NOT modify the parameter, observed_grid. **
    :param observed_grid: 2D list  observed counts
    ...
    #TODO: fill me in!
    row_list = []
    totalSum = total_sum(observed_grid)
    for i in range(len(observed_grid)):
        rowSum = row_sum(observed_grid, i)
        col_list = []
        for j in range(len(observed_grid[0])):
            colSum = col_sum(observed_grid, j)
            col_list.append(calculate_expected(rowSum, colSum, totalSum))
...
def perform_chi2_homogeneity_test(observed_grid):
    ...
    ** DO NOT CHANGE THE NAME OF THIS FUNCTION!! ** (this will mess with our autograder)
    ...
    '''
    #TODO: fill me in!
    dfChi2 = df_chi2(observed_grid)
    chi2Value = chi2_value(observed_grid)
    return 1 - chi2.cdf(chi2Value, dfChi2)
